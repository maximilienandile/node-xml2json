...
function text(data) {
    currentObject['value'] = (currentObject['value'] || '') + data;
}

function endElement(name) {
    if (currentObject['value']) {
        if (options.trim) {
            currentObject['value'] = currentObject['value'].trim()
        }

        if (options.sanitize) {
            currentObject['value'] = sanitizer.sanitize(currentObject['value'], true);
        }

        currentObject['value'] = coerce(currentObject['value'],name);
    }
    ...
    if (!options.reversible) {
        if (('value' in currentObject) && (Object.keys(currentObject).length == 1)) {
            if (ancestor[name] instanceof Array) {
                ancestor[name].push(ancestor[name].pop()['value']);
            } else {
                ancestor[name] = currentObject['value'];
            }
        }
    }
 ...
 *                characterized by the presence of the property value.
